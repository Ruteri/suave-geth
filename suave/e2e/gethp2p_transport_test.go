...
func TestGethP2PTransport(t *testing.T) {
	fr1 := newFramework(t, WithExecutionNode(), WithGethP2PTransportOpt())
	t.Cleanup(fr1.Close)
	...
	fr2 := newFramework(t, WithExecutionNode(), WithGethP2PTransportOpt())
	t.Cleanup(fr2.Close)

	// Connect the nodes
	ch := make(chan *p2p.PeerEvent, 1)
	subs := fr1.suethSrv.node.Server().SubscribeEvents(ch)
	fr1.suethSrv.node.Server().AddPeer(fr2.suethSrv.node.Server().Self())
